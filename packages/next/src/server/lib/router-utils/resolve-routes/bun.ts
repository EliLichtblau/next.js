...
export function getResolveRoutes(
    ...
    async function resolveRoutes({
        ...
        const initUrl = (config.experimental as any).trustHostHeader
            ...
            : opts.port
                ? `${protocol}://${formatHostname(opts.hostname || 'localhost')}:${opts.port
                }${req.url}`
                : req.url || ''
        
     
        addRequestMeta(req as RevisitEli, 'initURL', initUrl)
        addRequestMeta(req as RevisitEli, 'initQuery', { ...parsedUrl.query })
        addRequestMeta(req as RevisitEli, 'initProtocol', protocol)
        ...
        if (config.i18n) {
            ...
            domainLocale = detectDomainLocale(
                config.i18n.domains,
                getHostNameFromRequest(parsedUrl, req)
            )
        ...
        async function handleRoute(
            ...
            if ((route.has || route.missing) && params) {
                const hasParams = matchHas(
                    // TODO: avoid calling that multiple times
                    {headers: (req.headers as any).toJSON() as Record<string, string>},
                    parsedUrl.query,
                    route.has,
                    route.missing
                )
            ...
            if (params) {
                ...
                if (!opts.minimalMode && route.name === 'middleware') {
                    ...
                    if (
                        ...
                        addRequestMeta(req as RevisitEli, 'invokePath', '')
                        addRequestMeta(req as RevisitEli, 'invokeOutput', '')
                        addRequestMeta(req as RevisitEli, 'invokeQuery', {})
                        addRequestMeta(req as RevisitEli, 'middlewareInvoke', true)
                        ...
                        if (middlewareHeaders['x-middleware-override-headers']) {
                            ...
                            // Delete headers.
                            const headers = (req.headers as any).toJSON() as Record<string, string>
                            ...
                            for (const key of overriddenHeaders.keys()) {
                                ...
                                const oldValue = req.headers.get(key)

                                if (oldValue !== newValue) {
                                    if (newValue !== null && newValue !== undefined) {
                                        req.headers.set(key, Array.isArray(newValue) ? newValue.join(", ") : newValue)
                        ...
                        for (const [key, value] of Object.entries({
                            ...
                            if (value) {
                                ...
                                req.headers.set(key, Array.isArray(value) ? value.join(",") : value)
                            }
                        }
                ...
                if (route.destination) {
                    ...
                    try {
                        ...
                        if (isInterceptionRouteRewrite(route as Rewrite)) {
                            const stateHeader = req.headers.get(NEXT_ROUTER_STATE_TREE_HEADER.toLowerCase())
